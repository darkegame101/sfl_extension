...
## 🛠️ Quy tắc Debug: "Breadcrumb Logging" (Bắt buộc)
Mọi quy trình phức tạp (Giao hàng, Tương tác NPC) phải tuân thủ việc ghi log từng bước (Step-by-Step) vào Console:
- **Sử dụng Icon**: `🔍` (Kiểm tra), `🎯` (Hành động), `🔥` (Kết quả), `✅` (Thành công), `❌` (Lỗi).
- **Ghi số bước**: Ví dụ `[BƯỚC 1/3]: Đang nhắm bắn tọa độ`.
- **Minh bạch**: Phải giải thích rõ TẠI SAO (ví dụ: "Lỗi do Codex che mắt" thay vì chỉ ghi "Lỗi").
